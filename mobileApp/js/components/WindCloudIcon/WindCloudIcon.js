...
class WindCloudIcon extends React.Component {
	...
	componentDidMount() {
		this.state.cloudPushTimer.addListener(({ value }) => {
			this.flushTransform(this.cloudRef, this.state.cloudScale, this.state.cloudTranslateX, this.state.cloudTranslateY)
		})
		...
		this.state.windTimer.addListener(({ value }) => {
			this.flushWindTransform([this.windOneRef, this.windTwoRef, this.windThreeRef], this.state.windVal)
		})
